...
bacteria <- open_dataframe(rds_file = "dehalogenators.rds" ) # OPENS DATAFRAME
################################################################
## THIS BLOCK OF CODE MAKES GRAPHS FOR Dehalococcoides sp. #####
################################################################
# SUBSET TO ONLY Dehaloccoides
...
t <- keyword_df(i, keyword = "3_transposase")
D <- rbind(d,i,t)
size_bp = 1500000
...
gg1l <- p#genome_ggbio(my_ideogram, my_pos, geom_call= "rect" , alpha = 1)
pdf("/Users/koshlan/Dropbox/Dehalogenation_1/ChapterDrafts/FinalFigureProduction/dhc_lines.pdf")
gg1l
dev.off()
# MAKE ONE GRAPH WITH POINTS
p <- ggplot() + layout_circle(my_ideogram, geom = "ideo", fill = "gray70", radius = 30, trackWidth = 4)
p <- p + layout_circle(my_ideogram, geom = "scale", size = 5, radius = 35, trackWidth = 2)
p <- p + layout_circle(my_pos, geom = "point", 
                       aes(y = bacteria, colour = keyword, fill = keyword, group = bacteria), 
                       alpha = 0.5 ,linetype = "blank" , radius = 14 , trackWidth = 16, size = 3) + 
  scale_size(range = c(1, 5))
gg1p <- p
pdf("/Users/koshlan/Dropbox/Dehalogenation_1/ChapterDrafts/FinalFigureProduction/dhc_points.pdf")
gg1p
dev.off()
##############################
#### END Dehalococcoides #####
...
#############################################################
## THIS BLOCK OF CODE MAKES GRAPHS FOR Dehalobacter sp. #####
#############################################################
...
gg2l <- p # genome_ggbio(my_ideogram, my_pos, geom_call= "rect" , alpha = 1)

### PDF ###
pdf("/Users/koshlan/Dropbox/Dehalogenation_1/ChapterDrafts/FinalFigureProduction/dhb_lines.pdf")
gg2l
dev.off()
### PDF ###
...
p <- p + layout_circle(my_pos, geom = "point", 
                       aes(y = bacteria, colour = keyword, fill = keyword, group = bacteria), 
                       alpha = 0.5 ,linetype = "blank" , radius = 22 , trackWidth = 6, size = 3) + scale_size(range = c(1, 5))
gg2p <- p # genome_ggbio(my_ideogram, my_pos, geom_call= "rect" , alpha = 1)

### PDF ###
pdf("/Users/koshlan/Dropbox/Dehalogenation_1/ChapterDrafts/FinalFigureProduction/dhb_points.pdf")
gg2p
dev.off()
### PDF ###
###########################
...
#############################################################
## THIS BLOCK OF CODE MAKES GRAPHS FOR Dehalogenimonas sp. ##
#############################################################
...
p <- p + layout_circle(my_pos, geom = "rect", 
  ...
  scale_size(range = c(1, 5))
gg3l <- p #genome_ggbio(my_ideogram, my_pos, geom_call= "rect" , alpha = 1, track_width_call = 10)

### PDF ###
pdf("/Users/koshlan/Dropbox/Dehalogenation_1/ChapterDrafts/FinalFigureProduction/dhg_lines.pdf")
gg3l
dev.off()
### PDF ###

# MAKE ONE GRAPH WITH CIRCLES
p <- ggplot() + layout_circle(my_ideogram, geom = "ideo", fill = "gray70", radius = 30, trackWidth = 4)
p <- p + layout_circle(my_ideogram, geom = "scale", size = 5, radius = 35, trackWidth = 2)
p <- p + layout_circle(my_pos, geom = "point", 
                       aes(y = bacteria, colour = keyword, fill = keyword, group = bacteria), 
                       alpha = 0.5 ,linetype = "blank" , radius = 24 , trackWidth = 6, size = 3) + 
  scale_size(range = c(1, 5))

gg3p <- p #genome_ggbio(my_ideogram, my_pos, geom_call= "rect" , alpha = 1, track_width_call = 10)
### PDF ###
pdf("/Users/koshlan/Dropbox/Dehalogenation_1/ChapterDrafts/FinalFigureProduction/dhg_points.pdf")
gg3p
dev.off()
### PDF ###
...
### BELOW SHOWS HOW MAKE RAPID BUT UNDESSPECTIFIED PLOTS @@@
